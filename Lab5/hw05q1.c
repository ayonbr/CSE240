...
int add(char* employeeName_input, char* supervisorName_input, char* department_input, unsigned int roomNumber_input){
    ...
    {
        //if inputted string is already included, then do not add again
    ...
    //converting to enum type departmentType
    if(strcmp(department_input, "HR"))
    {
        list[count].department = HR;
    }
    else if(strcmp(department_input, "Marketing"))
    {
        list[count].department = Marketing;
    }
    else{
        list[count].department = IT;
    }
    //increment number of elements in list after employee is successfully added
    count++;
...
void display()
{
    ...
    if(count == 0){
        ...
        //displaying full output depending on the department type
        for(int i=0;i<count;i++)
        {
...
{	
    ...
    for(int i=0;i<count;i++)
    {
        //j = i+1 (next element to compare to previous)
        for(int j = i+1;j<count;j++)
        {
            //swap values if next value is greater than previous
            if(strcmp(list[i].employeeName, list[j].employeeName) > 0)
            {
                employeeTemp = list[i];
                list[i] = list[j];
                list[j] = employeeTemp;
...
// So at the beginning of this function, write code to open the file and check if it exists. If file does not exist, then return from the function.
...
void load(char* fileName)
{
    ...
    if(fileBuffer != NULL){
        ...
        for(int i=0;i<count;i++)
        {
            //reading employee record to file
            fread(list[i].employeeName, sizeof(list[i].employeeName),1, fileBuffer);
            fread(list[i].supervisorName, sizeof(list[i].supervisorName),1, fileBuffer);
            fread(&departmentValue, sizeof(departmentValue),1, fileBuffer);
            fread(&list[i].roomNumber, sizeof(list[i].roomNumber),1, fileBuffer);
